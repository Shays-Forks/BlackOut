...
public class BedAuraPlus extends BlackOutModule {
    ...
    private boolean placeUpdate() {
        ...
        if (rotMode.get() == RotationMode.Packet) {
            sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(bedDir.getOpposite().getPositiveHorizontalDegrees(), Managers.ROTATION.lastDir[1], Managers.ON_GROUND.isOnGround(), false));
        } else {
            Managers.ROTATION.startYaw(bedDir.getOpposite().getPositiveHorizontalDegrees(), priority, RotationType.Other, Objects.hash(name + "placing"));
            if (Math.abs(RotationUtils.yawAngle(Managers.ROTATION.lastDir[0], bedDir.getOpposite().getPositiveHorizontalDegrees())) > 45) {
                return false;
            }
        }
